...
createMaps <- function(rastDA,rastS,rastM,varX,unitsX){
  ...
  dat1 <- data.table(counts= f1[which(!is.na(f1))],offset="s2019 - DA2019")
  dat2 <- data.table(counts= f2[which(!is.na(f2))],offset="m2019 - DA2019")
  dat <- rbind(dat1,dat2)
...
for(jx in 1:3){ #loop variables
  for(i in 1:length(tileXs)){ #loop tiles
    ...
    f1 <- getValues(dev$m2019)
    f2 <- getValues(dev$s2019)
    dat1 <- data.table(counts= sample(f1[which(!is.na(f1))],1e6),offset="m2019 - DA2019")
    dat2 <- data.table(counts= sample(f2[which(!is.na(f2))],1e6),offset="s2019 - DA2019")
    dat <- rbind(dat1,dat2)
    dat$offset <- as.factor(dat$offset)
    ...
    # datAll[[z]] <- dat
    # 
    # pX <- ggplot(dat, aes(x=counts, color=offset)) +
    #   geom_histogram(fill="white", alpha=0.5, position="identity") +
    #   ylab(NULL) +# xlab(paste0(varXs[jx]," offset ",unitsX[jx])) +
    ...
    # plist[[z]]  <- pX
    # z = z+1
    # histX[[jx]][[i]] <- ggplot(dat, aes(x=counts, color=offset)) +
    #   geom_histogram(fill="white", alpha=0.5, position="identity") +
    #   ylab(NULL) + xlab(paste0(varXs[jx]," offset ",unitsX[jx])) +
    #   theme(axis.title.y=element_blank(),
    #         axis.text.y=element_blank(),
  ...
  # figX[[jx]] <- ggarrange(histX[[1]], histX[[2]],histX[[3]],
